...
class List_Content_Combine_RandomController: NSViewController, NSTableViewDelegate, NSTableViewDataSource{
    ...
    override func viewDidLoad() {
        ...
        mtheme = UserDefaults.standard.object(forKey: "hint_category_for_serch") as! String
        
        let stocks = realm.objects(Hint_Db.self).filter("theme == %@",mtheme)
        var temp :[String] = []
        for one in stocks{
            temp.append(one.content)
        }
    ...
    @objc func select_theme(){
        if select_stock != ""{
            ...
            UserDefaults.standard.set(select_stock, forKey: "hint_content")
            UserDefaults.standard.set(m_hint_theme_num, forKey: "hint_content_num")
            UserDefaults.standard.set(true, forKey: "return_from_List_Content_Combine_Random")
